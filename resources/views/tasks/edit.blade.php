@extends('layouts.app')
@section('title', 'Taak bewerken – BABB Portaal')

@section('content')
<div class="mb-6 flex items-center gap-3">
    <a href="{{ route('tasks.index') }}" class="text-gray-400 hover:text-gray-700 text-sm">&larr; Taken</a>
    <h1 class="text-2xl font-bold text-gray-900">Taak bewerken</h1>
</div>
...
<form method="POST" action="{{ route('tasks.update', $task) }}">
    ...
    <div class="mt-6 flex items-center gap-3">
        ...
        <form method="POST" action="{{ route('tasks.destroy', $task) }}"
              onsubmit="return confirm('Taak verwijderen?')" class="ml-auto">
            @csrf @method('DELETE')
            <button type="submit" class="text-sm text-red-500 hover:text-red-700">Verwijderen</button>
        </form>
    </div>
</form>
@endsection
